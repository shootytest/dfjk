...
export const math = {
  ...
  lerp_angle: (a: number, b: number, t: number) => {
    return vector.angle(vector.add(vector.mult(vector.rad_to_vector(a), (1 - t)), vector.mult(vector.rad_to_vector(b), t)));
  },
  ...
  lane_hit_x: (lanes: number, lane: number): number => {
    if (lanes === 4) return lane;
      ...
      if (lane === 3) return 5;
      else if (lane > 3) return lane - 1;
    }
    else if (lanes === 6) {
      if (lane === 1) return 5;
      else if (lane === 6) return 6;
      else return lane - 1;
    }
    else if (lanes === 7) {
      if (lane === 1) return 5;
      else if (lane === 7) return 6;
      else if (lane === 4) return 7;
      else if (lane > 4) return lane - 2;
      else return lane - 1;
    }
    return lane;
  },
