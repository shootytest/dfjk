export const settings = {
  
  NOTESPEED: 20,
  current_chart: {
    chart_name: "beeps",
    song_id: 0,
    song_type: 0,
  },
  offset: 0,
  line_offset: 0,
  hit_volume: 5,
  play_speed: 1,
  display_number: "score",
  play_mode: "play",
  controls: "dfjk",

  get notespeed() {
    return this.NOTESPEED / 20 / settings.play_speed;
  },
  get normal_mode() {
    return this.play_mode === "play";
  },
  get view_mode() {
    return this.play_mode === "view";
  },
  get practice_mode() {
    return this.play_mode === "practice";
  }

};

export const config = {
  version: "0.7.0.1",
  charts: 23,
  lanes: 4, // ?!
  cdn_i: "https://res.cloudinary.com/dzzjrhgkb/image/upload/v1742913250/dfjk/",
  cdn_v: "https://res.cloudinary.com/dzzjrhgkb/video/upload/v1742913252/dfjk/",
};

export const main_menu = {

};

export const songs = [
  {
    id: 0,
    name: "calibration",
    bpm: 120,
    types: ["calibration", "", "", "", "", "calibration "],
    difficulties: [1, -1, -1, -1, -1, 1],
    charts: ["beeps", "", "", "", "", "beeps_"],
    notes: [600, -1, -1, -1, -1, 100],
    preview: "beeps_preview",
    image: "beeps.png",
    fourmode: 1, // number of difficulty types in 4-key mode
  },
  {
    id: 1,
    name: "piano_music_01.mp3",
    bpm: 121.5,
    types: ["easy", "medium", "hard", "extreme", "", "easy "],
    difficulties: [3, 6, 17, 48, -1, 5],
    charts: ["saloon_1", "saloon_2", "saloon_3", "saloon_4", "", "saloon_1_"],
    notes: [100, 148, 228, 390, -1, 100],
    preview: "saloon_preview",
    image: "saloon.png",
    fourmode: 4,
  },
  {
    id: 2,
    name: "Deep Under",
    bpm: 63,
    types: ["easy", "medium", "hard", "", "", "easy "],
    difficulties: [5, 12, -1, -1, -1, -1],
    charts: ["deepunder_1", "deepunder_3", "deepunder_2", "", "", "deepunder_1_"],
    notes: [401, 606, 0, -1, -1, 0],
    preview: "deepunder_preview",
    image: "deepunder.png",
    fourmode: 3,
  },
  {
    id: 3,
    name: "Loneliness",
    bpm: 120.2,
    types: ["easy", "medium", "hard", "", "", "easy "],
    difficulties: [6, 14, 32, -1, -1, -1],
    charts: ["loneliness_1", "loneliness_2", "loneliness_3", "", "", "loneliness_1_"],
    notes: [472, 689, 1061, -1, -1, 0],
    preview: "loneliness_preview",
    image: "loneliness.jpg",
    fourmode: 3,
  },
  {
    id: 6,
    name: "Happiness",
    bpm: 126,
    types: ["easy", "medium", "hard", "", "", "easy "],
    difficulties: [5, 9, 21, -1, -1, -1],
    charts: ["happiness_1", "happiness_2", "happiness_3", "", "", "happiness_1_"],
    notes: [507, 600, 1017, -1, -1, 0],
    preview: "happiness_preview",
    image: "happiness.png",
    fourmode: 3,
  },
  {
    id: 4,
    name: "dusk approach",
    bpm: 90,
    types: ["easy", "medium", "hard", "", "", "easy "],
    difficulties: [2, 9, -1, -1, -1, -1],
    charts: ["dusk_1", "dusk_2", "dusk_3", "", "", "dusk_1_"],
    notes: [170, 400, 0, -1, -1, 0],
    preview: "dusk_approach_preview",
    image: "dusk_approach.jpg",
    fourmode: 3,
  },
  {
    id: 5,
    name: "⠞⠧⠶⠳⡇⠼⠗",
    bpm: 74.63,
    types: ["easy", "medium", "hard", "", "", "easy "],
    difficulties: [4, 8, 26, -1, -1, -1],
    charts: ["tetris_1", "tetris_2", "tetris_3", "", "", "tetris_1_"],
    notes: [189, 303, 666, -1, -1, 0],
    preview: "tetris_preview",
    image: "tetris.png",
    fourmode: 3,
  },
  {
    id: 8,
    name: "▲ ⯁ ⬤ ◼",
    bpm: 120,
    types: ["easy", "medium", "hard", "", "", "easy "],
    difficulties: [6, -1, -1, -1, -1, -1],
    charts: ["kahoot_1", "kahoot_2", "kahoot_3", "", "", "kahoot_1_"],
    notes: [276, 0, 0, -1, -1, 0],
    preview: "kahoot_preview",
    image: "kahoot.jpg",
    fourmode: 3,
  },
  {
    id: 7,
    name: "🧪🥧✳️",
    bpm: 120,
    types: ["easy", "medium", "hard", "", "", "easy "],
    difficulties: [2, 7, -1, -1, -1, -1],
    charts: ["nush_1", "nush_2", "nush_3", "", "", "nush_1_"],
    notes: [173, 333, 0, -1, -1, 0],
    preview: "nush_preview",
    image: "nush.jpg",
    fourmode: 3,
  },
];

export interface chart_metadata {
  chart_name: string,
  song_id: number,
  song_type: number,
};

export interface chart_metadata_2 {
  chart_name: string,
  song_name: string,
  song_id: number,
  song_type: string,
  song_difficulty: number,
};

export const chart_map: { [key: string]: chart_metadata_2 } = {};
for (const song of songs) {
  const id = song.id;
  for (let i = 0; i < song.types.length; i++) {
    const name = song.charts[i];
    if (name.length <= 0) continue;
    chart_map[name] = {
      chart_name: name,
      song_name: song.name,
      song_id: id,
      song_type: song.types[i].trim(),
      song_difficulty: song.difficulties[i],
    };
  }
}

export const skill_rate_data = (function() {
  const raw = `
    1010000	✪	1	1.8	0
    1009500	✸	500	1.795	0.005
    1009000	★★	500	1.78	0.015
    1008000	★	1000	1.75	0.03
    1004000	✦✦	4000	1.65	0.1
    1000000	✦	4000	1.5	0.15
    995000	SS+	5000	1.36	0.14
    990000	SS	5000	1.23	0.13
    985000	S+	5000	1.11	0.12
    980000	S	5000	1	0.11
    965000	AA+	15000	0.9	0.1
    950000	AA	15000	0.83	0.07
    925000	A+	25000	0.74	0.09
    900000	A	25000	0.65	0.09
    875000	B+	25000	0.56	0.09
    850000	B	25000	0.47	0.09
    800000	C+	50000	0.33	0.14
    750000	C	50000	0.25	0.08
    650000	D+	100000	0.2	0.05
    500000	D	150000	0.1	0.1
    250000	E	250000	0.025	0.075
    1	F	249999	0	0.025
    0	Z	1	0	0
  `;
  const result = [];
  for (const line of raw.trim().split("\n")) {
    const [s, r, ds, m, dm] = line.trim().split("\t");
    const s_ = parseInt(s);
    const ds_ = parseInt(ds);
    const m_ = parseFloat(m);
    const dm_ = parseFloat(dm);
    result.push({
      score: s_,
      rank: r,
      d_score: ds_,
      mult: m_,
      d_mult: dm_,
    });
  }
  return result;
})();

export const special_skill_rate_data = [0, 0.1, 0.15, 0.2, 0.2];

export const skill_rate = function(score: Score) {
  let result = chart_map[score.chart]?.song_difficulty ?? 0;
  const value = score.value;
  const special = score.special;
  for (const a of skill_rate_data) {
    if (a.score <= value) {
      const mult = a.mult + ((value - a.score) / a.d_score * a.d_mult) + special_skill_rate_data[special];
      return result * mult; // runs once
    }
  }
  return result;
};

export const variance_rate = function(variance: number) {
  if (variance <= 25) return "✪";
  if (variance <= 75) return "✸";
  if (variance <= 85) return "★★";
  if (variance <= 100) return "★";
  if (variance <= 120) return "✦✦";
  if (variance <= 150) return "✦";
  if (variance <= 170) return "SS+";
  if (variance <= 200) return "SS";
  if (variance <= 220) return "S+";
  if (variance <= 250) return "S";
  if (variance <= 270) return "AA+";
  if (variance <= 300) return "AA";
  if (variance <= 325) return "A+";
  if (variance <= 360) return "A";
  if (variance <= 400) return "B+";
  if (variance <= 450) return "B";
  if (variance <= 500) return "C+";
  if (variance <= 600) return "C";
  if (variance <= 700) return "D+";
  if (variance <= 1000) return "D";
  if (variance <= 1500) return "E";
  if (variance > 5000) return "Z";
  return "F";
};

export interface Score { // i think i used lowercase "score" for variable names already (so it won't look nice? `score: score` like vision ratios)
  chart: string,
  //song_id: number, // id of song
  //song_type: number, // type index
  value: number,
  max_combo: number,
  special: number,
  skill: number,
  time?: number,
};

export const scores = {

  list: [] as Score[], // don't use
  map: {} as { [ key: string ]: Score[] },
  peak_skill: 0,
  total_skill: 0,
  load: () => {
    const raw = localStorage.getItem("scores");
    if (raw) {
      const o = zipson.parse(raw);
      if (Array.isArray(o)) {
        scores.list = o;
        scores.load_map_from_list();
      } else {
        scores.map = zipson.parse(raw);
      }
      // console.log(scores.map);
    } else {
      scores.save();
    }
    scores.recalculate_skills();
  },
  save: () => {
    localStorage.setItem("scores", zipson.stringify(scores.map));
  },
  clear: () => {
    scores.list = [];
    scores.map = {};
    scores.total_skill = 0;
    scores.save();
  },
  add: (score: Score) => {
    const difficulty_number = chart_map[score.chart].song_difficulty;
    const old = scores.map[score.chart];
    if (old == undefined) {
      scores.list.push(score);
      scores.map[score.chart] = [score];
    } else {
      old.push(score);
      old.sort(scores.compare_fn);
      if (old.length > 10) {
        old.length = 10;
      }
    }
    // scores.save();
  },
  load_map_from_list: () => { // deprecated
    for (const k in scores.map) {
      delete scores.map[k];
    }
    scores.map = {};
    for (const score of scores.list) {
      if (!scores.map[score.chart]) scores.map[score.chart] = [];
      scores.map[score.chart].push(score);
    }
    for (const scorelist of Object.values(scores.map)) {
      scorelist.sort(scores.compare_fn);
    }
  },
  compare_fn: (a: Score | Object & { skill: number }, b: Score | Object & { skill: number }) => {
    return b.skill - a.skill;
  },
  check_same: (a: Score, b: Score): boolean => {
    return a.chart === b.chart && a.max_combo === b.max_combo && a.special === b.special && a.value === b.value && a.time === b.time;
  },
  check_contains: (sl: Score[], b: Score): boolean => {
    for (const a of sl) {
      if (scores.check_same(a, b)) return true;
    }
    return false;
  },
  get_list: () => {
    const result: Score[] = [];
    for (const scorelist of Object.values(scores.map)) {
      result.push(scorelist[0]);
    }
    result.sort(scores.compare_fn);
    return result;
  },
  recalculate_skills: () => {
    scores.peak_skill = 0;
    scores.total_skill = 0;
    for (const k in scores.map) {
      const scorelist = scores.map[k];
      for (const score of scorelist) {
        score.skill = skill_rate(score);
      }
      scorelist.sort(scores.compare_fn);
      if (scorelist[0]) {
        const s = scorelist[0].skill;
        scores.total_skill += s;
        if (s > scores.peak_skill) scores.peak_skill = s;
      }
    }
    scores.save();
  },
  init: () => {
    scores.load();
  },

};


export interface Requirement {
  name: string,
  at_least: number,
  charts: string[],
  scores?: number[],
  combos?: number[],
  specials?: number[],
};

export interface Requirements {
  name: string,
  at_least: number,
  requirements: (string | Requirement)[],
}

type Lock = Requirement | Requirements;

export const requirements: { [key: string]: Lock } = {
  /*["6"]: {
    name: "score AA on loneliness",
    at_least: 1,
    requirements: ["happiness_1", "happiness_2", "happiness_3"],
  },*/
  saloon_4: {
    name: "score S on this song's hard",
    at_least: 1,
    charts: ["saloon_3"],
    scores: [980000],
  },
  happiness_2: {
    name: "score S on loneliness easy",
    at_least: 1,
    charts: ["loneliness_1"],
    scores: [980000],
  },
  happiness_3: {
    name: "score S on loneliness medium",
    at_least: 1,
    charts: ["loneliness_2"],
    scores: [980000],
  },
  happiness_4: {
    name: "score S on loneliness hard",
    at_least: 1,
    charts: ["loneliness_3"],
    scores: [980000],
  },
  nush_3: {
    name: "???",
    at_least: 1,
    charts: ["nush_3"],
    scores: [1],
  },
};

export const requirement = {

  check: function(r?: Lock) {
    if (r == undefined) return true;
    if (r.at_least <= 0) return true;
    let fulfilled = 0;
    if ("requirements" in r) {
      r = r as Requirements;
      for (const s of r.requirements) {
        if (requirement.check(typeof s === "string" ? requirements[s as string] : s)) fulfilled += 1;
        if (fulfilled >= r.at_least) return true;
      }
    } else {
      r = r as Requirement;
      for (let i = 0; i < r.charts.length; i++) {
        const k = r.charts[i];
        const scorelist = scores.map[k];
        if (scorelist == undefined || scorelist.length <= 0) continue;
        for (const s of scorelist) {
          if (s.value >= (r.scores?.[i] ?? 0) && s.max_combo >= (r.combos?.[i] ?? 0) && s.special >= (r.specials?.[i] ?? 0)) {
            fulfilled += 1;
            break;
          }
        }
        if (fulfilled >= r.at_least) return true;
      }
    }
    return false;
  },

};