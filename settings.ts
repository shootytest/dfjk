...
export const scores = {
  ...
  add: (score: Score) => {
    ...
    if (old == undefined) {
      ...
      if (score.value > old.value) old.value = score.value;
      if (score.max_combo > old.max_combo) old.max_combo = score.max_combo;
      if (score.special > old.special) old.special = score.special;
      if (score.skill > old.skill) old.skill = score.skill;
    }
    scores.save();
  },
  load_map: () => {
