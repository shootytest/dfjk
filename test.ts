...
import { Chart, charts, code_to_lane } from "./chart.js";
...
const init = function() {
  ...
  key.add_key_listener("Space", function() {
    if (ui.menu === "game" && Chart.current?.type === "full" && Sound.current && Sound.current?.play_timestamp !== -1) return;
  ...
  key.add_key_listener("KeyR", function() {
    if (ui.menu === "game" && Chart.current?.type === "full") return;
    if (document.activeElement?.tagName.toLowerCase() === "input") return;
    ui.restart();
  });
