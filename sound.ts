...
export class Sound {
  ...
  playing_delay: boolean = false;
  play_timestamp: number = -1; // negative = haven't started playing yet
  pause_timestamp: number = -1;
  pause_time_total: number = 0;
  ...
  time_to(other_timestamp: number) {
    return other_timestamp - this.time;
  }

  time_of(timestamp: number) {
...
for (let i = 1; i <= 10; i++) {
  ...
  (sfx_make["hit" + i] as sfxr.sound_options).sound_vol = 0.2 * Math.pow(0.5, 6 - (i - 1) / 10 * 6);
}

export const sfx: { [key: string]: sfxr.Sound } = { };
