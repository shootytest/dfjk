...
export const ui = {
  ...
  draw_list: function(v: vector) {
    const size = { x: ui.width, y: ui.height };
    const x_constrained: boolean = size.y >= size.x * 1.6;
    ...
    ctx.beginPath();
    if (x_constrained) ctx.rect(0, 0, size.x, v.y);
    else {
      x = v.x - 0.15 * r;
  ...
  list_change_index: function(delta: number) {
  ...
  draw_board: function(v: vector, size: vector, x_constrained: boolean = false) {
    ...
    for (const note of Object.values(chart.active_notes)) {
      ...
      if (note.hit >= 0) {
        ...
        if (t < 200) {
          if (ui.game.hit_force) { // todo: hit force currently not working / not nice
            // ui.draw_effect(new Effect(["y", ui.game.hit_force, note.hit_time, 200], chart), -t);
  ...
  draw_effect: function(e: Effect, time_to: number) {
    ...
    if (type === "tilt") {
      ...
      if (ui.game.separation_target === e.a) return;
  ...
  make_toplist: function(uid?: string, name?: string) {
    ...
    */
    const fn = function(list: Score[], others = true) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        for (let j = 0; j <= score.special; j++) special_count[j]++;
        const d = new Date();
      ...
      const span_skill = document.getElementById("totalskill") as HTMLSpanElement;
      span_skill.textContent = totalskill.toFixed(3);
      span_skill.title = "" + totalskill;
      const span_score = document.getElementById("totalscore") as HTMLSpanElement;
      span_score.textContent = totalscore.toFixed(0);
      span_score.title = "" + totalscore;
      for (let j = 1; j <= 4; j++) {
        const span_special = document.getElementById("special" + j) as HTMLSpanElement;
      ...
      if (!others) ui.hide_box("toplist");
      else document.getElementById("toplist-back")?.addEventListener("click", function() {
        ui.close_boxes();
        ui.show_box("leaderboard");
      });
    };
    ...
    if (leaderboard) {
      ...
      firebase.get_scores(chart_name, (leaderboard) => {
        ...
        if (!firebase.signed_in || !firebase.user) {
          ...
          if (list && list[0]) leaderboard.push({
            uid: "",
            username: "you",
      ...
      if (!list || list.length === 0) {
        ...
        main.innerHTML = `
          ...
          <p> <button id="switch"> switch to leaderboard </button> </p>
        `;
        const table = document.getElementById("chair_chart") as HTMLTableElement;
        for (let i = 0; i < list.length; i++) {
          const score = list[i];
          const tr = document.createElement("tr");
          const gr = Chart.grade(score.value);
